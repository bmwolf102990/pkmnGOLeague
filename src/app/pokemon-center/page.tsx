        ...
        <>
            ...
            <div className="z-0 flex flex-col h-auto w-screen bg-black">
                ...
                <section id="registration-section" className="flex flex-col w-screen text-justify text-sm xl:text-lg min-[2000px]:text-2xl font-light px-3 xl:px-6 min-[2000px]:px-12 min-[600px]:text-center py-4 xl:py-8 min-[2000px]:py-16">
                    ...
                    <form className="grid grid-col-2 gap-2 py-2 justify-center border rounded text-center">
                        <input className="col-span-1 p-1 rounded-sm" type="text" placeholder="First Name" required/>
                        <input className="col-span-1 p-1 rounded-sm" type="text" placeholder="Last Name"required/>
                        <input className="col-span-2 p-1 rounded-sm" type="text" placeholder="Trainer Name" required/>
                        <input className="col-span-2 p-1 rounded-sm text-black" type="text" placeholder="Trainer Code" pattern="\d{4}\s\d{4}\s\d{4}" title="#### #### ####" required/>
                        <input className="col-span-2 p-1 rounded-sm" type="email" placeholder="Email" required/>
                        <input className="col-span-2 p-1 rounded-sm" type="password" placeholder="Password" required/>
                        <input className="col-span-2" type="file" accept=".jpeg, .jpg, .png" required/>
                        <button className="col-span-2 bg-green-500 hover:bg-green-300 text-black font-bold hover:outline hover:outline-offset-2 hover:outline-green-300 py-2 px-4 rounded">
                            Submit
                        </button>
                    </form>
                </section>
